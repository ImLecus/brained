...
export const GraphView = memo(function GraphView({
  ...
  const drawHalo = (
    ...
    if (String(node.id) !== hovered || node.x === undefined || node.y === undefined) {
      return;
    }
    const radius =
  ...
  useEffect(() => {
    ...
    const zoomBy = (factor: number, clientX: number, clientY: number) => {
      ...
      const point = graph.screen2GraphCoords(clientX - rect.left, clientY - rect.top);
    ...
    const move = (event: MouseEvent) => {
      if (cam.middle) {
        ...
        cam.middle.lastY = event.clientY;
        if (dy !== 0) {
          zoomBy(1 + dy * MIDDLE_SENSITIVITY, event.clientX, event.clientY);
        }
        return;
      }
      if (!cam.pan) {
    ...
    <div ref={container} className="graph-area">
      {size.width > 0 && size.height > 0 && (
        <ForceGraph2D
          ...
          nodeColor={(node) => (String(node.id) === hovered ? invertColor : color)}
          nodeCanvasObjectMode={() => "before"}
          nodeCanvasObject={drawHalo}
          linkColor={(link) => (highlighted(link) ? color : withAlpha(color, 0.6))}
